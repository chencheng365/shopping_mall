    ...
    <div class="container">
      <div class="checkout-addr">
        ...
        <div class="addr-list-wrap">
          <div class="addr-list">
            <ul>
              <li v-for="(item,index) in addressListFirter" :key="index" v-bind:class="{'check':checkIndex==index}" @click="checkIndex=index;selectedAddrId=item.addressId">
                <dl>
                  <dt>{{item.userName}}</dt>
                  <dd class="address">{{item.detailedAddress}}</dd>
...
export default {
  ...
  methods:{
    init(){
      axios.get("https://www.easy-mock.com/mock/5a9e6826430ae46225384d8a/example/addressList")
      .then(result=>{
    ...
    setDefault(addressId){
      ...
      // let defaultID = this.addressList.filter(i=>i.productId==addressId)
      // defaultID[0].isDefault=true;
    ...
    delAddress(){
      this.addressList = this.addressList.filter(i=>i.addressId!=this.addressId);
